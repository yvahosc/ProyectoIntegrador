...
public class AccountController {
    ...
    public ResponseEntity<DTOAccountTransfer> transferAccount(HttpServletRequest request, @RequestBody DTOAccountTransfer dtoAccountTransfer) throws ApiException {
        this.accountService.isLoggedInUserTheOwnerOfTheAccount(dtoAccountTransfer.getSourceAccountNumber(),
                request.getUserPrincipal().getName());
        return ResponseEntity.ok(this.accountService.transferToAccount(dtoAccountTransfer));
    }

    @GetMapping("/{accountNumber}/pockets")
    public ResponseEntity<List<DTOPocketConsultIn>> getPockets (HttpServletRequest request, @PathVariable Long accountNumber) throws ApiException {
